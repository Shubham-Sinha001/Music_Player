//let me select all required tags or elments
...
window.addEventListener("load", ()=>{
    loadMusic(musicIndex);      // calling the load music function  once window is loaded
    playingNow();
})

// function for load music
...
// next music function
function nextMusic(){
    //this fn juch increment the index by 1
    musicIndex++;
...
// prev music function
function prevMusic(){
    //this fn juch decrement the index by 1
    musicIndex--;
...
nextBtn.addEventListener("click", ()=>{
    nextMusic();                           // calling next music fn
});

//prev music btn event
prevBtn.addEventListener("click", ()=>{
    prevMusic();                           // calling next music fn
});

//update progress bar with according to the music current timing
mainAudio.addEventListener("timeupdate", (e)=>{
    const currentTime = e.target.currentTime;           // getting current time of song
    const duration = e.target.duration;                //// getting total time duration of song
    ...
    let musicCurrentTime = wrapper.querySelector(".current"),
         ...
         mainAudio.addEventListener("loadeddata", ()=>{ //  update song total duration
        ...
        //  update playing song current timing
...
// let's update playiing song current tiome on according to the progress bar width
...
repeatBtn.addEventListener("click", ()=>{
    ...
    //let's do differnt changes on differnt icon click using switch
    switch(getText){
...
//Above we just changes the icon ,now let's work on what to do 
// After the song endedd
...
mainAudio.addEventListener("ended", ()=>{
    ...
    //let's do differnt changes on differnt icon click using switch
    switch(getText){
        case "repeat":     // if this icon is repeat then simply we      call  the nextMusic fn so that next song can play in repitive manner
            nextMusic();
             break;
        case "repeat_one":   // if icon is repeat_one then the current playing song current timeto 0 so song will play from begining 
        ...
        }while(musicIndex == randomIndex);  // this loop run untill the next random number won't be the same of current music index
...
for (let i = 0; i < allMusic.length; i++) {
    ...
    let liTag = ` <li li-index="${i + 1}">
             ...
             <span  id="${allMusic[i].src}" class="audio-duration">8:50</span>
            </li>`;
...
//getting li index of particulae clicked li tag
